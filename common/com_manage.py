...
class ComManage:
    ...
    def validate_manner(self, validates_dict):
        ...
        try:
            ...
            logging.error("解析validate数据失败，数据为：{validates_dict}")
            raise es
    ...
    def request_manner(self, request_dict):
        ...
        try:
            ...
            logging.error("请求发送失败，请求信息是：{request_dict}")
            raise ("请求发送失败，请求信息是：{request_dict}")
    ...
    def variables_value(self, yaml_path, yaml_name, variables_data):
        ...
        try:
            ...
            logging.error("被依赖用例{yaml_name}不存在依赖用例所需的依赖数据：{variables_data}")
            raise ("被依赖用例{yaml_name}不存在依赖用例所需的依赖数据：{variables_data}")
...
if __name__ == '__main__':
    ...
    #print(yaml_path)
    datas = ComParams().test_params(yaml_path, yaml_name="testone.yaml")
    #print(datas)
    for data in datas:
       #print(data[0])
